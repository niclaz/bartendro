...
0. Merge content from the many separate repositories
   * [x] `mayhem/bartendro-config`: integrate readme.md from there into /docs
   * [ ] `mayhem/bartendro-config` review the scripts that affect networking - have the RPi setup an AP using available tools on RPI 4
   * [ ] `mayhem/bartendro-config` review the scripts that point to code running on other machines (this repo /scripts/start_bartendro.sh
   * [ ] `mayhem/bartendro-deploy` is there anything of use herein?
   * [ ] `mayhem/bartendro-chef-cookbooks` is there anything of use herein?
   * [ ] 
...
2. Serve its own wifi access point
   ...
   * [ ] setup script should automate the selection of raspi-config options necessary for project:
     ...
     * [ ] Advanced: Enable I2C
3. Fix annoying bug where drinks will be poured directly from the drinks menu, without waiting for the confirmation screen
...
4. Setup wizard - current setup is complicated and lots of manual work, requires figuring out combinations on paper
...
6. Create new .img release after tested as working on Raspberry Pi 4
...
7. Test if Bartendro works for other versions of Raspberry Pi OS
